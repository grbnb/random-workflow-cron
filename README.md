...
| github_token | string  |  `${{ github.token }}` | The GitHub PAT key with at least `repo` and `workflow` permissions [Personal Access Token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token). |
...
| keep_history | boolean | false | Keep except workflow_file history commit message outside of the file, please ensure that the depth of checkout is set to 2, `fetch-depth: 2` |
